...
def test(map, epochs, learner, verbose):
    ...
    startpos = getrobotpos(map)  # find where the robot start
    ...
    for epoch in range(1, epochs+1):
        ...
        action = learner.querysetstate(state)  # set state and get first action
        count = 0
        while (robopos != goalpos) & (count < 10000):
            # move to new loc according to action and then get a new action
...
def test_code():
    ...
    learner = ql.QLearner(num_states=100, num_actions=4, alpha=0.2,
                          gamma=0.9, rar=0.98, radr=0.999, dyna=0,
                          verbose=False)
    epochs = 500
